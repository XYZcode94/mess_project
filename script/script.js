// script.js (with Professional Cart)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchProducts(startAfterDoc = null, searchTerm = '') {
        ...
        try {
            ...
            if (isSearch) {
                ...
                q = query(productsCollection, orderBy('name'), where('name', '>=', searchTermCapitalized), where('name', '<=', searchTermCapitalized + '\uf8ff'));
                loadMoreBtn.classList.add('hidden');
            } else {
                q = startAfterDoc ? query(productsCollection, orderBy('name'), startAfter(startAfterDoc), limit(productsPerPage)) : query(productsCollection, orderBy('name'), limit(productsPerPage));
            }
    ...
    searchForm.addEventListener('submit', (e) => { e.preventDefault(); const searchTerm = searchInput.value.trim(); addToSearchHistory(searchTerm); renderSkeletonLoader(); fetchProducts(null, searchTerm); searchInput.blur(); hideSearchHistory(); });
    searchInput.addEventListener('input', (e) => { if (e.target.value === '') { lastVisibleDoc = null; renderSkeletonLoader(); fetchProducts(); } });
...
function toggleMenu() { const navLinks = document.getElementById('nav-links'); navLinks.classList.toggle('active'); }
